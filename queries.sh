...
echo "$($PSQL "SELECT DISTINCT name FROM teams LEFT JOIN games ON teams.team_id=games.winner_id WHERE year=2018 AND winner_id=205")"
...
echo "$($PSQL "
  ...
  WHERE games.year = 2014
    AND games.round IN ('Eighth-Final', 'Eighth Final')
  ORDER BY name;
")"
...
echo "$($PSQL "SELECT DISTINCT year, name FROM teams JOIN games ON teams.team_id=games.winner_id WHERE round='Final' ORDER BY year")"
